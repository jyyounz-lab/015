...
網頁 APP 內也有 `本機 AI 字幕助手`，可以輸入影片完整路徑、輸出資料夾、模型與語言，直接產生可複製的 PowerShell 指令。瀏覽器基於安全限制無法直接執行本機程式或讀取完整磁碟路徑，因此仍需要將指令貼到 PowerShell 執行。
若先選擇本機影片，網頁可取得檔名；再貼上資料夾路徑後，可用 `用已選影片補上檔名` 與 `使用影片所在資料夾` 減少手動輸入。

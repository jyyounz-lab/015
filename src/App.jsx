...
function drawSubtitle(ctx, text, width, height) {
  if (!text) return;
  ctx.save();
  ...
  const y = height - 132;
  drawRoundRect(ctx, x, y, boxWidth, 76, 12);
  ctx.fillStyle = "rgba(0,0,0,0.68)";
  ctx.fill();
  ctx.strokeStyle = "rgba(0,0,0,0.78)";
  ctx.lineWidth = 5;
  ctx.strokeText(text, width / 2, y + 39);
  ctx.fillStyle = "#fff";
  ctx.fillText(text, width / 2, y + 39);
  ctx.restore();
}
...
export default function App() {
  ...
  const [subtitles, setSubtitles] = useState([{ id: id(), text: "第一段字幕", start: 0, end: 3 }]);
  ...
  function jump(amount) {
    const video = previewRef.current;
    if (!video || !activeClip) return;
    video.currentTime = clamp(video.currentTime + amount, activeClip.start, activeClip.end);
  }
  ...
  async function exportVideo() {
    ...
    for (let index = 0; index < clips.length; index += 1) {
      ...
      while ((performance.now() - startAt) / 1000 < segmentDuration) {
        ...
        drawSubtitle(ctx, activeSubtitle(currentTimeline)?.text, canvas.width, canvas.height);
            ...
            {activeClip ? (
              <>
                ...
                <div className="overlay">
                  {activeSubtitle(previewRef.current?.currentTime || 0)?.text && (
                    <div className="subtitle">{activeSubtitle(previewRef.current?.currentTime || 0)?.text}</div>
                  )}
          ...
          <section>
            ...
            {subtitles.map((subtitle) => (
              <div className="subtitleRow" key={subtitle.id}>
                ...
                />
                <button onClick={() => setSubtitles((rows) => rows.filter((row) => row.id !== subtitle.id))}>
                  <Trash2 size={15} />
                </button>
              </div>
            ))}
            <button className="button light full" onClick={() => setSubtitles((rows) => [...rows, { id: id(), text: "新字幕", start: 0, end: 3 }])}>
              <Plus size={16} />
              新增字幕
            </button>
          </section>
